$(document).ready(function () {
    ...
    $('#collection_datetime').datetimepicker({}).on('dp.change', function (e) {
        //$('#collection_datetime').parsley().validate();
    });

    $('#last_due_datetime').datetimepicker().on('dp.change', function (e) {
        // $('#last_due_datetime').parsley().validate();
    });

    /* $(document).on('change', '#cust_code', function (evt, params) {
     var selectedCustomerCode = $('#cust_code option:selected').text();
     updateCustomerInfo(selectedCustomerCode, 'code');
     });*/
    
    /*new code*/
    //var selectedCustomerCode = $('#cust_code option:selected').text();  // get selecetd customer code
    //updateCustomerInfo(selectedCustomerCode, 'code');
    ...
    //when the Add cheque information button is clicked
    $("#add_more_cheque").click(function (e) {
        $("#tbl_cheque").append('<tr><td><input type="text" id="reference" name="reference[]" class="form-control"  value="" /></td><td><input type="text" id="invoice" name="invoice[]" class="form-control" value="" /></td><td><input type="text" id="po" name="po[]" class="form-control" value="" /></td><td><input type="text" id="amount" name="amount[]" class="form-control text-right amount" value="" data-parsley-pattern="^\d+(\.\d{1,2})?$" data-parsley-pattern-message="Allow upto 2 decimal point. e.g: 4.00"/></td><td><a href="javascript:void(0);" class="btn btn-link delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td></tr>');
    });
