...
class Login extends CI_Controller {
    ...
    function check_database() {
        ...
        $match = "email = '" . $email . "' && password = '" . md5($password) . "' && status = 'active' && is_delete = 0";

        $result = $this->common_model->get_records(LOGIN, array("login_id, firstname, lastname, email, role_id"), '', '', $match);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($exitEmailId)) {
                ...
                if ($this->input->post('email')) {
                    ...
                    $subject = "codeigniter :: " . $template[0]['subject'];
